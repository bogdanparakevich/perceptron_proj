...
x, y_true, w0_model, w1_model,b = gen_2D(n_dots = 50, x_min = 0, x_max = 1, w0_model = 0.2, w1_model = 1, M = -0.5, std = 1)
...
plt.plot(x, perceptron_2D(x, y_true)[0] + perceptron_2D(x, y_true)[1] * x, 'r', label='true') # true 
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
plt.show()
